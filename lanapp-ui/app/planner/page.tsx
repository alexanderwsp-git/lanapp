...
export default function PlannerPage() {
  ...
  function cancelRow(row: BreedingRecord) {
    if (row.result !== "Pendiente" || row.diagnosisDate) {
    ...
    setRows((prev) =>
      prev.map((r) => (r.id === row.id ? { ...r, status: "Cancelado" as const } : r)),
    )
  }
                  ...
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-sm">
                      <div className="flex flex-wrap gap-2">
                        ...
                        {r.result === "Pendiente" && !r.diagnosisDate && (
                          <button
                            type="button"
                            onClick={() => cancelRow(r)}
