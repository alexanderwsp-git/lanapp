...
function drawerMeta(scheduleOnly: boolean, confirmingPlanned: boolean) {
  ...
  if (confirmingPlanned) return { title: "Registrar monta", submit: "Confirmar monta" }
...
export function MatingRegisterDrawer({
  ...
  const blockedForRegister =
    !confirmingPlanned && !form.scheduleOnly && !!blockReason

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (blockedForRegister) return
    setSaving(true)
    setError(null)
    try {
        ...
        blockedForRegister ? (
          ...
          <>
            ...
            <button
              ...
              disabled={
                saving ||
                (!(form.scheduleOnly && isFemale) && !form.partnerId)
              }
        ...
        <form id="mating-register-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {confirmingPlanned ? (
            <p className="rounded-md bg-indigo-50 px-3 py-2 text-sm text-indigo-800">
              Monta programada en el planificador ({plannedCycle?.cycleName}). Confirma que ocurrió
              en campo o ajusta los datos.
            </p>
          ) : (
            <SwitchField
          ...
          <Field
            ...
            required={!(form.scheduleOnly && isFemale)}
          >
            <Combobox
              id="mating-partner"
