 ...
 */
export function SheepReproStats({ sheep }: { sheep: ApiSheep }) {
  const [matings, setMatings] = useState<ApiMating[]>([])
  const [offspring, setOffspring] = useState<ApiSheep[]>([])
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    Promise.all([fetchMatingsBySheep(sheep.id), fetchSheep({ page: 1, limit: 500 })])
      .then(([m, all]) => {
        if (cancelled) return
        setMatings(m)
        const kids = all.items.filter((s) =>
          isMale ? s.fatherId === sheep.id : s.motherId === sheep.id,
        )
        setOffspring(kids)
      })
      .catch(() => {
